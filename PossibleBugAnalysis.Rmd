...
This report describes a series of small issues that can converge such that an `DESeq2` analysis runs from start to finish and raises no errors, but silently corrupts and invalidates the data, producing results that may or may not seem feasible. I observed it in practice and detected it using DESeq2's quality control functions. It seemed all-too-plausible that the situation could arise in other projects, so I've documented it for study. If nothing else, it can serve as a reminder of some easily forgotten but counter-intuitive and consequential default behavior of R.
...
Though we're passing `factor`s, `DESeq2` will raise an error stating that the user has passed `character`s and should be passing numeric.

A `character` can be safely cast as an `integer`, but a `factor` cannot; R will silently convert to an ordered sequence, completely invalidating the data (ie, the sequence 1, 100, 1000 is changed to 1, 2, 3).

This new, incorrect dataset will run without errors, but is basically unrelated to what the user intends to analyze. 

*Note:* a bug report has been filed and the error message updated. Click [here](https://support.bioconductor.org/p/130310/#130371) for link.
...
When we read the data from a file, the well-known-but-still-menacing factor default issues comes in to play. However, `DESeq2` appears to misdiagnose the types. To see how, we first read in the file with the bad row in it. 
...
error <- tryCatch(
  {
    # try to use the bad one, pass as matrix for sake of consistency
    dds <- DESeqDataSetFromMatrix(countData = ctsBadFile %>% as.matrix(),
